...
create table `user`(
    ...
    `create_time` TIMESTAMP INT NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_uid` (`user_id`),
...
create table `team`(
    ...
    `avatar` varchar(128) COMMENT '头像',
    `status` int NOT NULL COMMENT '状态',
    `update_time` TIMESTAMP NOT NULL DEFAULT TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
